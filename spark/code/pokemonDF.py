...
#TO START
# ./sparkSubmitPython.sh pokemonDF.py "org.apache.spark:spark-streaming-kafka-0-8_2.11:2.4.5,org.elasticsearch:elasticsearch-hadoop:7.7.0"
...
csv_schema = tp.StructType([
    tp.StructField(name = 'pokemons', dataType=tp.StringType(), nullable=True),
    tp.StructField(name = 'win', dataType=tp.LongType(), nullable=True)
])

def createDF(rowRdd, schema):
    return spark.createDataFrame(rowRdd, schema=schema)

pokeDF = spark.createDataFrame(spark.sparkContext.emptyRDD(), schema = battleSchema)
...
def get_prediction_json(key,rdd):
    ...
    # battleDataFrame.write.csv("/home/alem/showdown_tap/spark/dataset/my_csv.csv")
